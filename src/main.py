...
if __name__ == "__main__":
    ...
    parser.add_argument('--csv_path',default= '../parler-hate-speech/parler_annotated_data_bt.csv')
    ...
    if args.pretrain_hate:
        ...
        if args.classification:
            train3[CFG.target_cols] = (train3[CFG.target_cols] > 0.4999).values.astype(int)
    ...
    train = pd.read_csv(args.csv_path) #implicit_hate_binary.csv
    ...
    if args.classification:
        train[CFG.target_cols] = (train[CFG.target_cols] > 0.4999).values.astype(int)
    Fold = MultilabelStratifiedKFold(n_splits=CFG.n_fold, shuffle=True, random_state=CFG.seed)
